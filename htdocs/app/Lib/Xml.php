	...
	public static function build($input, $options = array()) {
		...
		if (is_array($input) || is_object($input)) {
			...
			if ($options['return'] === 'simplexml' || $options['return'] === 'simplexmlelement') {
				try {
					$_return = new SimpleXMLElement($input, LIBXML_COMPACT);
					return $_return;
				} catch (Exception $e) {
					throw new XmlException(__d('cake_dev', $e->getMessage()));
				}
			}
			$dom = new DOMDocument();
			$dom->loadXML($input);
			...
			if ($options['return'] === 'simplexml' || $options['return'] === 'simplexmlelement') {
				try {
					$_return = new SimpleXMLElement($input, LIBXML_COMPACT, true);
					return $_return;
				} catch (Exception $e) {
					throw new XmlException(__d('cake_dev', $e->getMessage()));
				}
